...
* The [OrderEndpoints.cs](src/Order.ServiceApi/Endpoints/OrderEndpoints.cs) is responsible for exposing an Order API to the outer world.
* The [OrderUseCase.cs](src/Order.ServiceApi/UseCases/OrderUseCase.cs) is responsible for implementing the business logic around the creation of orders.
* The [PastryAPIClient.cs](src/Order.ServiceApi/Client/PastryAPIClient.cs) is responsible for calling the Pastry API in Product Domain and get details or list of pastries.
...
* The Order API is specified using the [order-service-openapi.yaml](src/Order.AppHost/resources/order-service-openapi.yaml) OpenAPI specification,
* The Pastry API is specified using the [apipastries-openapi.yaml](src/Order.AppHost/resources/third-parties/apipastries-openapi.yaml) OpenAPI specification,
* The Order Events are specified using the [order-events-asyncapi.yaml](src/Order.AppHost/resources/order-events-asyncapi.yaml) AsyncAPI specification.
